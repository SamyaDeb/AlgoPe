...
export class PaymentClient {
  ...
  constructor(options: PaymentClientOptions) {
    ...
    //
    // This MUST be registered against the same concrete CAIP-2 id used for the
    // algo-exact scheme above, not the default "algorand:*" wildcard. The core
    // client resolves a network by exact-match first and only falls back to a
    // wildcard when there is no exact entry at all — so registering algo-exact
    // under the concrete id would shadow a wildcard-registered exact and make
    // every USDC-priced service unpayable.
  ...
  async fetch<T = unknown>(
    ...
    if (response.status === 402) {
      ...
      // Record the payment. The settlement txid only exists on the retry
      // response (PAYMENT-RESPONSE header), so it has to be read here — the
      // 402 challenge alone cannot supply it.
  ...
  private readSettlement(
    response: Response
  ): { transaction?: string; payer?: string; success?: boolean } | undefined {
    try {
      const settle = this.httpClient.getPaymentSettleResponse((name: string) =>
        response.headers.get(name)
      );
      return settle as { transaction?: string; payer?: string; success?: boolean } | undefined;
    } catch {
      return undefined;
    }
  }
  ...
  private recordPayment(
    ...
    settlement?: { transaction?: string; payer?: string; success?: boolean }
  ): PaymentReceipt {
