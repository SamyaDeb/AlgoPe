...
import { AlgoPeAgent } from "./agent.js";
import {
  isKeychainAvailable,
  hasMnemonic,
  getCredentialInfo,
...
program
  .name("algope-agent")
  .description("AlgoPe Agent — Pre-built AI agent with x402 payments on Algorand (Secure Keychain Storage)")
  .version(VERSION);

program
  .command("init")
  .description("Configure the AlgoPe agent (stores wallet in OS keychain)")
  .action(runInit);
