...
/**
 * Load local registry from ~/.algope/registry.json
 */
async function loadLocalRegistry(): Promise<ServiceInfo[]> {
  try {
    const registryPath = path.join(os.homedir(), ".algope", "registry.json");
...
export function createDiscoverServiceTool(options: DiscoverServiceToolOptions) {
  ...
  return tool({
    description:
      "Search for AI services in the AlgoPe marketplace. Use this to find services that can help with a task. Returns service names, descriptions, prices, and endpoints.",
