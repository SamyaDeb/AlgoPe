...
export interface AlgoPeAgentOptions {
  config?: AgentConfig;
  verbose?: boolean;
}
  ...
  private async ensureInitialized(): Promise<void> {
    ...
    if (!mnemonic) {
      throw new Error(
        ...
        `Run 'algope-agent init' to set up your wallet, or 'algope-agent migrate-keychain' ` +
      ...
      callPaidApi: tool({
        ...
        execute: async ({ serviceName, path, method, body }) => {
          ...
          try {
            ...
            let service = null;
            try {
              const configPath = pathMod.join(os.homedir(), ".algope", "config.json");
...
Provide a clear answer based on the API response.`;
   ...
   */
  static async fromConfig(): Promise<AlgoPeAgent> {
    ...
    const agent = new AlgoPeAgent({ config });
 ...
 */
export function createAgent(options?: AlgoPeAgentOptions): AlgoPeAgent {
