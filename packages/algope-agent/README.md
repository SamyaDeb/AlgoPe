...
AlgoPe Agent is an AI agent SDK that can discover, pay for, and use monetized API services. Built on the Vercel AI SDK with support for OpenAI and Anthropic.
...
```bash
algope-agent run "Find a weather service and get the weather for San Francisco"
```
...
| Command | Description |
|---------|-------------|
| `algope-agent init` | Interactive setup wizard |
| `algope-agent status` | Show config and wallet balance |
| `algope-agent services` | List available services |
| `algope-agent run <task>` | Execute a task with the agent |
...
The agent stores config in `~/.algope/agent.json`:
