# AlgoPe Consumer Developer Setup
...
```bash
algope-agent run "Get weather for London"
```
...
|---------|-------------|---------|
| `algope-agent init` | Interactive setup wizard | `algope-agent init` |
| `algope-agent status` | Show config, wallet balance | `algope-agent status` |
| `algope-agent services` | List available paid services | `algope-agent services` |
| `algope-agent run <task>` | Execute a task | `algope-agent run "analyze data"` |
| `algope-agent run <task> --provider <name>` | Use specific service | `algope-agent run "get weather" --provider Weather` |
| `algope-agent run <task> --verbose` | Show payment details | `algope-agent run "task" --verbose` |
...
// Create agent (reads config from ~/.algope/agent.json)
...
- Check balance: `algope-agent status`
- Minimum 0.1 ALGO recommended

### "Service not found"
- Check services: `algope-agent services`
...
2. **Explore available services** - `algope-agent services`
...
AlgoPe makes it easy for AI agents to autonomously discover and pay for services using real blockchain micropayments.
