...
import os from "os";
var WALLET_DIR = path.join(os.homedir(), ".algope");
...
function buildState() {
  ...
  const registryAppId = process.env.ALGOPE_REGISTRY_APP_ID ?? wallet?.registryAppId ?? void 0;
...
// src/tools/bazaar-search.ts
function toAlgoPeNetwork(network) {
  return network === "algorand" ? "mainnet" : "testnet";
}
function registerBazaarSearch(server2, config2) {
  server2.tool(
    "search_bazaar",
    "Discover x402-gated services registered on the AlgoPe on-chain registry (Algorand). Returns each service with its description, price, tags, and the endpoint URL. To use a service, pass its endpoint to the `x402_fetch` tool, which handles the USDC/ALGO payment automatically.",
    {
    ...
    async ({ query, tags, maxPrice }) => {
      const net = toAlgoPeNetwork(config2.network);
      try {
        ...
        if (services.length === 0) {
          ...
          return {
            content: [
              {
                type: "text",
                text: `No AlgoPe services found${where} on ${net} (registry app ${client.getAppId()}).`
              }
            ]
          };
        }
   ...
   Endpoint: ${s.endpoint}`
        ...
        const header = query ? `Found ${services.length} AlgoPe service(s) matching "${query}":` : `Found ${services.length} AlgoPe service(s):`;
        const summary = `
...
${serviceList}${summary}`
        ...
        return {
          content: [
            {
              type: "text",
              text: `AlgoPe registry search failed: ${err instanceof Error ? err.message : String(err)}`
            }
          ],
          isError: true
        };
