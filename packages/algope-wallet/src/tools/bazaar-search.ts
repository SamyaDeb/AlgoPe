 ...
 * AlgoPe registry (AlgoPeRegistry contract on Algorand).
 *
...
import { RegistryClient, type AlgoPeNetwork } from '@/algope-registry.js'

/** Maps the wallet's payment network to a AlgoPe registry network. */
function toAlgoPeNetwork(network: PaymentNetwork): AlgoPeNetwork {
...
export function registerBazaarSearch(server: McpServer, config: AppConfig): void {
  server.tool(
    'search_bazaar',
    'Discover x402-gated services registered on the AlgoPe on-chain registry ' +
    ...
    async ({ query, tags, maxPrice }) => {
      const net = toAlgoPeNetwork(config.network)
      ...
      try {
            ...
            content: [
              {
                type: 'text' as const,
                text:
                  `No AlgoPe services found${where} on ${net} ` +
        ...
        const header = query
          ? `Found ${services.length} AlgoPe service(s) matching "${query}":`
          : `Found ${services.length} AlgoPe service(s):`
        ...
        return {
          content: [
            {
              type: 'text' as const,
              text: `AlgoPe registry search failed: ${
