/**
 * AlgoPe Registry Reader (self-contained)
 *
 * Reads service registrations from the on-chain AlgoPeRegistry contract on
 * Algorand. Ported from packages/algope-agent/src/registry.ts and trimmed to a
 * single dependency (algosdk) so it can be bundled into the .mcpb wallet.
 *
 * App ID priority:
 *   1. ALGOPE_REGISTRY_APP_ID environment variable
 *   2. Default fallback (769455464 — deployed registry)
 */
...
const DEFAULT_APP_ID = 769455464n
...
export type AlgoPeNetwork = 'testnet' | 'mainnet'
export type RegistryPaymentToken = 'ALGO' | 'USDC'

export interface AlgoPeService {
  id: string
  name: string
  description: string
  tags: string[]
...
/**
 * Resolves the registry App ID from ALGOPE_REGISTRY_APP_ID, falling back to the
 * deployed default.
 */
 ...
 */
function decodeServiceData(data: Uint8Array): AlgoPeService {
  ...
  return {
    ...
    walletAddress,
    network: network as AlgoPeNetwork,
...
export class RegistryClient {
  ...
  private network: AlgoPeNetwork
  private indexerUrl: string

  constructor(network: AlgoPeNetwork = 'testnet') {
  ...
  async findService(
    developerAddress: string,
    name: string
  ): Promise<AlgoPeService | undefined> {
   ...
   */
  async listAllServices(): Promise<AlgoPeService[]> {
    ...
    const servicePromises: Promise<AlgoPeService | null>[] = []
    ...
    return services.filter((s): s is AlgoPeService => s !== null)
  }

  /** Lists all services then applies search filters. */
  async search(options: SearchOptions = {}): Promise<AlgoPeService[]> {
