...
> **AlgoPe** on-chain registry — using the **x402** micropayment protocol.

AlgoPe Wallet is a [Model Context Protocol](https://modelcontextprotocol.io)
...
Algorand-only and rewired so discovery reads the on-chain **AlgoPeRegistry**
contract instead of Pixa's hosted marketplace.

## How it works

1. **`search_bazaar`** reads the AlgoPeRegistry (Algorand app, default
   `769455464`) via the indexer and lists registered services with their price
   and endpoint.
...
| AlgoPe registry app id | `ALGOPE_REGISTRY_APP_ID` | `769455464` |
...
npm run build:mcpb   # produces algope.mcpb (double-click to install in Claude Desktop)
npm test             # vitest
```
