...
AlgoPe is a reverse proxy that sits in front of your HTTP API and enforces x402 protocol payments on every request. One command, zero backend changes, real ALGO/USDC settlements on Algorand.
...
| Command | Description |
|---------|-------------|
| `algope init` | Interactive setup wizard |
| `algope start` | Start the x402 payment proxy |
| `algope register` | Register service on Algorand |
| `algope list` | List registered services |
| `algope status` | Show current config and wallet balance |

## How It Works

```
Client Request → AlgoPe Proxy → 402 Payment Required
                     ↓
...
```typescript
import { startProxyServer } from "@algope/cli";
