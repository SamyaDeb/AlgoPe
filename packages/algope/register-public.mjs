...
let config
try {
  const raw = await readFile(join(homedir(), '.algope', 'config.json'), 'utf-8')
  config = JSON.parse(raw)
} catch {
  console.error('Error: Run "algope init" first to set up your service config.')
  process.exit(1)
}
...
const appId = BigInt(config.registryAppId ?? '769455464')
