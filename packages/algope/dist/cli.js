...
var GOPLAUSIBLE_FACILITATOR_URL = "https://facilitator.goplausible.xyz";
...
// src/facilitator/goplausible.ts
import { HTTPFacilitatorClient } from "@x402-avm/core/server";
var PROBE_TIMEOUT_MS = 8e3;
function caip2For(network) {
  if (network === "testnet") return ALGORAND_TESTNET_CAIP2;
  if (network === "mainnet") return ALGORAND_MAINNET_CAIP2;
  throw new Error(
    `Unsupported network "${network}". AlgoPe settles on Algorand \u2014 expected "testnet" or "mainnet". Fix the "network" field in ~/.algope/config.json (or re-run "algope init").`
  );
}
function schemeForToken(token) {
...
function createGoPlausibleFacilitator(options) {
  ...
  return {
    ...
    async verify(payload, requirements) {
      try {
        ...
        return {
          isValid: false,
          invalidReason: `Facilitator verify failed (${url}): ${error.message}`
        };
      }
    },
    async settle(payload, requirements) {
      try {
        ...
        return {
          ...
          errorMessage: `Facilitator settle failed (${url}): ${error.message}`,
          transaction: "",
...
async function probeFacilitator(options) {
  ...
  try {
    ...
    return { ...base, reason: `unreachable: ${error.message}` };
  }
  ...
  const feePayer = forNetwork.map((kind) => kind.extra?.feePayer).find((value) => typeof value === "string");
  let networkStatus;
  try {
    const health = await fetchJson(`${url}/health`, headers, timeoutMs);
  ...
  if (!match) {
    return {
      ...
      feePayer,
      networkStatus,
      reason: `does not support the "${options.requiredScheme}" scheme (offers: ${schemes.join(", ") || "none"})`
    };
  }
...
var AlgoNativeFacilitator = class {
  ...
  constructor(walletAddress, network = "testnet") {
    ...
    this.indexer = new algosdk2.Indexer(
      "",
  ...
  async verify(payload, requirements) {
    try {
      ...
      const signedTxn = algosdk2.decodeSignedTransaction(signedTxnBytes);
      ...
      if (txn.type !== algosdk2.TransactionType.pay) {
        return {
          isValid: false,
  ...
  async settle(payload, requirements) {
    ...
    try {
      ...
      try {
        await algosdk2.waitForConfirmation(this.algod, txId, 4);
      } catch (confirmError) {
...
var SimplePaymentVerifier = class {
  ...
  async settle(payload, requirements) {
    ...
    try {
      ...
      const paymentTxId = algosdk3.decodeSignedTransaction(blobs[group.index]).txn.txID();
      try {
      ...
      await algosdk3.waitForConfirmation(this.algod, paymentTxId, 10);
      return {
        success: true,
  ...
  decodePayment(payload) {
    ...
    const stxn = algosdk3.decodeSignedTransaction(bytes);
    const txn = stxn.txn;
...
// src/facilitator/resolve.ts
async function resolveFacilitator(options) {
  const { config, facilitatorMnemonic } = options;
  const requested = config.facilitatorMode ?? "auto";
  ...
  if (probe.usable) {
    return {
      ...
      mode: "goplausible",
      description: `GoPlausible facilitator at ${probe.url}${probe.feePayer ? " (gasless \u2014 facilitator sponsors network fees)" : ""}`,
      feePayer: probe.feePayer
    };
  }
  if (requested === "goplausible") {
    throw new Error(
      `Remote facilitator ${probe.url} cannot settle this service: ${probe.reason}. ` + (requiredScheme === "algo-exact" ? `Native-ALGO pricing uses AlgoPe's "algo-exact" scheme, which the hosted facilitator does not implement \u2014 price in USDC to use it, or set facilitatorMode to "self".` : 'Set facilitatorMode to "self" or "local" to settle without it.')
    );
  }
  const fallback = selfSettle(config);
  return { ...fallback, fallbackReason: probe.reason };
}
function selfSettle(config) {
...
function formatRoutesForDisplay(routes) {
  ...
  return Object.entries(routesMap).map(([path4, config]) => {
    ...
    if (typeof price === "string" || typeof price === "number") {
      ...
      token = assetPrice.extra?.name || "USDC";
    } else {
      amount = "0";
      decimals = ALGO_DECIMALS;
      token = "ALGO";
...
var logServerStart = (port, serviceName) => {
  console.log();
  console.log(
    `${timestamp()} ${icons.server} ${chalk.magenta.bold("AlgoPe")} proxy started`
  );
  console.log(
...
async function createProxyServer(options) {
  ...
  app.use("/algope-admin", adminRouter);
  const networkCaip2 = config.network === "testnet" ? ALGORAND_TESTNET_CAIP2 : ALGORAND_MAINNET_CAIP2;
  const facilitator = await resolveFacilitator({ config, facilitatorMnemonic });
  logger.info(`Facilitator: ${facilitator.description}`);
  if (facilitator.fallbackReason) {
    logger.warn(
      `Remote facilitator unavailable (${facilitator.fallbackReason}) \u2014 settling locally instead.`
    );
  }
  if (facilitator.feePayer) {
    logger.verbose(`Fee payer: ${facilitator.feePayer}`);
  }
...
var ADMIN_ADDRESS = process.env.ALGOPE_ADMIN_ADDRESS || DEFAULT_ADMIN_ADDRESS;
...
function readAppIdFromConfig() {
  try {
    const configPath = path.join(os.homedir(), ".algope", "config.json");
...
var AlgoPeRegistryClient = class {
  algod;
  appId;
  appAddress;
...
var ADMIN_ADDRESS2 = process.env.ALGOPE_ADMIN_ADDRESS || DEFAULT_ADMIN_ADDRESS2;
var REGISTRATION_FEE2 = 1000000n;
var DEFAULT_APP_ID2 = 769455464n;
function readAppIdFromConfig2() {
  try {
    const configPath = path2.join(os2.homedir(), ".algope", "config.json");
  ...
  try {
    connector = new WalletConnectClient({
      ...
      clientMeta: {
        ...
        url: "https://algope.io",
        icons: ["https://algope.io/icon.png"]
      }
    });
    if (!connector) {
    ...
    if (accountBalance < totalRequired) {
      ...
      console.log();
      console.log(chalk2.gray("    Fund this wallet and run: algope register"));
...
// src/cli.ts
var CONFIG_DIR = path3.join(os3.homedir(), ".algope");
...
var VERSION = "1.0.0";
var algopeGradient = gradient(["#00D9FF", "#00FF94", "#FFD93D"]);
function getRegistryAppIdForConfig() {
  return process.env.ALGOPE_REGISTRY_APP_ID || DEFAULT_REGISTRY_APP_ID;
}
function printBanner() {
  console.log();
  console.log(
    algopeGradient.multiline(`
   _____ _           _       _____      
  / ____| |         (_)     |  __ \\     
...
async function runInit() {
  printBanner();
  p.intro(chalk3.bgCyan.black(" AlgoPe Setup "));
  ...
  const targetUrl = await p.text({
    message: "Your API URL (the backend AlgoPe will proxy)",
  ...
  if (p.isCancel(shouldRegister)) {
    console.log();
    console.log(chalk3.gray(`  You can register later with: ${chalk3.cyan("algope register")}`));
    console.log(chalk3.gray(`  Start the proxy with: ${chalk3.cyan("algope start")}`));
    console.log();
    process.exit(0);
  }
  if (shouldRegister) {
    ...
    if (p.isCancel(endpoint)) {
      ...
      const initRegistryClient = new AlgoPeRegistryClient(config.network);
      ...
      if (p.isCancel(registrationMethod)) {
        ...
        try {
          ...
          if (!result.success) {
            ...
            console.log(chalk3.gray(`  You can try again with: ${chalk3.cyan("algope register")}`));
            console.log();
          }
        } catch (error) {
  ...
  if (p.isCancel(shouldStart) || !shouldStart) {
    ...
    console.log(chalk3.white(`    ${chalk3.cyan("algope start")}`));
    console.log(chalk3.gray("  Or with local facilitator (no external server needed):"));
    console.log(chalk3.white(`    ${chalk3.cyan('algope start --facilitator "your 25 word mnemonic"')}`));
    console.log();
  ...
  const facilitatorMode = await p.select({
    ...
    options: [
      {
        value: "auto",
        label: "\u{1F310} GoPlausible facilitator (Recommended)",
        hint: "Hosted \u2014 gasless for payers, no key needed here"
      },
      {
        value: "self",
        label: "\u{1F517} Settle it yourself",
        hint: "Relay client-signed txns; the payer covers their own fee"
      },
      {
        value: "local",
        label: "\u{1F4BB} Local facilitator",
        hint: "Run in-process \u2014 needs a funded wallet"
      }
    ],
    initialValue: "auto"
  });
  let facilitatorMnemonic;
  if (!p.isCancel(facilitatorMode) && facilitatorMode !== "local") {
    config.facilitatorMode = facilitatorMode;
    await saveConfig(config);
  }
...
async function runStart(options) {
  ...
  if (!config) {
    ...
    console.log(chalk3.gray(`  Run ${chalk3.cyan("algope init")} first.`));
    process.exit(1);
  }
  if (options.port) {
  ...
  if (options.facilitatorMode) {
    config.facilitatorMode = options.facilitatorMode;
  }
  if (options.facilitatorUrl) {
    config.facilitatorUrl = options.facilitatorUrl;
  }
  if (options.facilitatorKey ?? process.env.ALGOPE_FACILITATOR_API_KEY) {
    config.facilitatorApiKey = options.facilitatorKey ?? process.env.ALGOPE_FACILITATOR_API_KEY;
  }
  console.log();
  try {
    const { facilitator } = await startProxyServer({
      config,
      facilitatorMnemonic: options.facilitator
    });
    console.log(chalk3.gray(`    Facilitator: ${chalk3.green(facilitator.description)}`));
    if (facilitator.fallbackReason) {
      console.log(
        chalk3.yellow(`    \u26A0 Remote facilitator unusable: ${facilitator.fallbackReason}`)
      );
    }
    console.log();
    console.log(chalk3.gray("  Press Ctrl+C to stop"));
...
async function runRegister() {
  ...
  const registryClient = new AlgoPeRegistryClient(config.network);
...
async function runList() {
  ...
  const config = await loadConfig();
  if (!config) {
    console.log(chalk3.gray("  Run algope init first."));
    console.log();
    return;
  }
  ...
  try {
    ...
    if (!svc) {
      ...
      console.log(chalk3.gray(`  Run ${chalk3.cyan("algope register")} to register your service.`));
      console.log();
      return;
    }
...
async function runStatus() {
  printBanner();
  const config = await loadConfig();
  console.log(chalk3.cyan.bold("\u25B8 AlgoPe Status"));
  console.log(chalk3.gray("\u2500".repeat(50)));
  console.log();
  if (!config) {
    console.log(chalk3.yellow("  \u26A0 Not configured"));
    console.log(chalk3.gray(`    Run ${chalk3.cyan("algope init")} to get started.`));
    console.log();
    return;
  }
  console.log(chalk3.gray("  Configuration:"));
  ...
  try {
    ...
    if (!isRegistered) {
      console.log(chalk3.gray(`    Run ${chalk3.cyan("algope register")} to list your service.`));
    }
  } catch {
    spinnerReg.stop();
...
program.name("algope").description("AlgoPe \u2014 Monetize any API with x402 micropayments on Algorand").version(VERSION);
program.command("init").description("Initialize a new AlgoPe configuration").action(runInit);
program.command("start").description("Start the x402 payment proxy").option("-p, --port <port>", "Override the proxy port").option("-v, --verbose", "Enable verbose logging").option("-f, --facilitator <mnemonic>", "25-word mnemonic for local facilitator (enables offline mode)").option(
  "-m, --facilitator-mode <mode>",
  "auto | goplausible | local | self (default: auto \u2014 GoPlausible with local fallback)"
).option(
  "-u, --facilitator-url <url>",
  "Facilitator base URL (default: https://facilitator.goplausible.xyz)"
).option("--facilitator-key <key>", "Bearer token for a gated facilitator").action(runStart);
