/**
 * AlgoPe Terminal WalletConnect Registration
 * 
...
function readAppIdFromConfig(): bigint | undefined {
  try {
    ...
    const config = JSON.parse(fs.readFileSync(configPath, "utf-8")) as AlgoPeConfig;
...
export async function registerWithWalletConnectTerminal(
  ...
  try {
    ...
    if (accountBalance < totalRequired) {
      ...
      console.log();
      console.log(chalk.gray("    Fund this wallet and run: algope register"));
