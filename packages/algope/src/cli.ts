...
import type { AlgoPeConfig, PaymentToken, ServiceRegistration, Registry } from "./types.js";
import {
...
import { AlgoPeRegistryClient } from "./registry.js";
...
const CONFIG_DIR = path.join(os.homedir(), ".algope");
const CONFIG_FILE = path.join(CONFIG_DIR, "config.json");
const REGISTRY_FILE = path.join(CONFIG_DIR, "registry.json");
const DEFAULT_REGISTRY_APP_ID = "769455464";

const VERSION = "1.0.0";

// Beautiful gradient for the banner
const algopeGradient = gradient(["#00D9FF", "#00FF94", "#FFD93D"]);
...
async function loadConfig(): Promise<AlgoPeConfig | null> {
  try {
    const data = await fs.readFile(CONFIG_FILE, "utf-8");
    return JSON.parse(data) as AlgoPeConfig;
  } catch {
    return null;
  }
}

async function saveConfig(config: AlgoPeConfig): Promise<void> {
...
async function runInit(): Promise<void> {
  ...
  p.intro(chalk.bgCyan.black(" AlgoPe Setup "));
  ...
  if (p.isCancel(shouldRegister)) {
    console.log();
    console.log(chalk.gray(`  You can register later with: ${chalk.cyan("algope register")}`));
    console.log(chalk.gray(`  Start the proxy with: ${chalk.cyan("algope start")}`));
    console.log();
    process.exit(0);
  }
      ...
      if (p.isCancel(registrationMethod)) {
        ...
        try {
          ...
          if (!result.success) {
            ...
            console.log(chalk.gray(`  You can try again with: ${chalk.cyan("algope register")}`));
            console.log();
          }
        } catch (error) {
  ...
  if (p.isCancel(shouldStart) || !shouldStart) {
    ...
    console.log(chalk.white(`    ${chalk.cyan("algope start")}`));
    console.log(chalk.gray("  Or with local facilitator (no external server needed):"));
    console.log(chalk.white(`    ${chalk.cyan("algope start --facilitator \"your 25 word mnemonic\"")}`));
    console.log();
  ...
  const facilitatorMode = await p.select({
    ...
    options: [
      {
        value: "auto",
        label: "🌐 GoPlausible facilitator (Recommended)",
  ...
  // Persist the choice so future `algope start` runs keep it. "local" is not
  // persisted: it needs a mnemonic supplied per-run via --facilitator.
  if (!p.isCancel(facilitatorMode) && facilitatorMode !== "local") {
    config.facilitatorMode = facilitatorMode as AlgoPeConfig["facilitatorMode"];
    await saveConfig(config);
  }
...
async function runStart(options: {
  ...
  if (!config) {
    ...
    console.log(chalk.gray(`  Run ${chalk.cyan("algope init")} first.`));
    process.exit(1);
  }
  ...
  // Facilitator overrides from flags take precedence over the saved config.
  if (options.facilitatorMode) {
    config.facilitatorMode = options.facilitatorMode as AlgoPeConfig["facilitatorMode"];
  }
  ...
  if (options.facilitatorKey ?? process.env.ALGOPE_FACILITATOR_API_KEY) {
    config.facilitatorApiKey =
      options.facilitatorKey ?? process.env.ALGOPE_FACILITATOR_API_KEY;
  }
  console.log();

  try {
    // The resolver preflights the facilitator, so the mode printed below is the
    // one actually in use — not a guess from the flags.
    const { facilitator } = await startProxyServer({
      config,
      facilitatorMnemonic: options.facilitator,
    });

    console.log(chalk.gray(`    Facilitator: ${chalk.green(facilitator.description)}`));
    if (facilitator.fallbackReason) {
      console.log(
        chalk.yellow(`    ⚠ Remote facilitator unusable: ${facilitator.fallbackReason}`)
      );
    }
    console.log();
...
async function runList(): Promise<void> {
  ...
  if (!config) {
    console.log(chalk.gray("  Run algope init first."));
    console.log();
    return;
  }
  ...
  try {
    ...
    if (!svc) {
      ...
      console.log(chalk.gray(`  Run ${chalk.cyan("algope register")} to register your service.`));
      console.log();
      return;
    }
...
async function runStatus(): Promise<void> {
  ...
  console.log(chalk.cyan.bold("▸ AlgoPe Status"));
  console.log(chalk.gray("─".repeat(50)));
  console.log();

  if (!config) {
    console.log(chalk.yellow("  ⚠ Not configured"));
    console.log(chalk.gray(`    Run ${chalk.cyan("algope init")} to get started.`));
    console.log();
    return;
  }
  ...
  try {
    ...
    if (!isRegistered) {
      console.log(chalk.gray(`    Run ${chalk.cyan("algope register")} to list your service.`));
    }
  } catch {
    spinnerReg.stop();
...
program
  .name("algope")
  .description("AlgoPe — Monetize any API with x402 micropayments on Algorand")
  .version(VERSION);

program
  .command("init")
  .description("Initialize a new AlgoPe configuration")
  .action(runInit);
...
program
  ...
  .option("--facilitator-key <key>", "Bearer token for a gated facilitator")
  .action(runStart);
