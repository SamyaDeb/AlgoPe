...
import type { ServiceRegistration, PaymentToken, AlgoPeConfig } from "./types.js";
...
// This can be overridden via ALGOPE_ADMIN_ADDRESS environment variable
const DEFAULT_ADMIN_ADDRESS = "CIQZP6I73Q5527QWZHZLZBIDSOHVV5LMP5IEQNQYVRXYOZTQSYB7X57PBE";
const ADMIN_ADDRESS = process.env.ALGOPE_ADMIN_ADDRESS || DEFAULT_ADMIN_ADDRESS;
...
/**
 * Reads the registry App ID from config file (~/.algope/config.json).
...
function readAppIdFromConfig(): bigint | undefined {
  try {
    ...
    const config = JSON.parse(data) as AlgoPeConfig;
...
export function encodeArc4String(s: string): Uint8Array {
 ...
 */
export function buildAppArgs(selector: Uint8Array, ...strings: string[]): Uint8Array[] {
 ...
 */
export function buildGetServiceArgs(
  selector: Uint8Array,
 ...
 */
export function decodeServiceData(data: Uint8Array): OnChainService {
...
export function buildBoxKey(senderAddr: string, serviceName: string): Uint8Array {
...
export class AlgoPeRegistryClient {
