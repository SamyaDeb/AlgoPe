...
import type {
  AlgoPeConfig,
  PaymentToken,
  RouteConfig as AlgoPeRouteConfig,
} from "../types.js";
import {
...
export function createRoutesConfig(
  config: AlgoPeConfig,
  additionalRoutes?: AlgoPeRouteConfig[]
): RoutesConfig {
...
export function formatRoutesForDisplay(
  ...
  return Object.entries(routesMap).map(([path, config]) => {
    ...
    if (typeof price === "string" || typeof price === "number") {
      ...
      const assetPrice = price as {
        amount: string;
        asset?: string;
        extra?: { decimals?: number; name?: string };
      };
      amount = assetPrice.amount;
      decimals = assetPrice.extra?.decimals || USDC_DECIMALS;
      // `asset` is the numeric ASA id (e.g. "10458941"); the display label
      // lives in extra.name. Using `asset` here printed the raw id in the CLI.
