/**
 * AlgoPe Weather API (zero-dependency backend)
 *
 * A simple weather backend you can monetize with AlgoPe. It has NO npm
 * dependencies — just Node's built-in http server — so it runs anywhere.
 *
 * This is the BACKEND that AlgoPe proxies. You do NOT register this URL
 * on-chain directly; you register the AlgoPe x402 proxy that wraps it.
 *
 * Run:
 *   node examples/algope-weather-api.mjs            # listens on :3001
 *   PORT=4000 node examples/algope-weather-api.mjs  # custom port
 *
 * Endpoints:
...
function weatherFor(cityRaw) {
  ...
  if (CITIES[key]) return { ...CITIES[key], source: 'AlgoPe Weather (sample data)' }
  ...
  return {
    ...
    humidity: 60,
    wind: 4.0,
    source: 'AlgoPe Weather (generated)'
  }
}
...
const server = http.createServer((req, res) => {
  ...
  if (path === '/health') {
    return send(res, 200, { status: 'healthy', service: 'AlgoPe Weather API', time: new Date().toISOString() })
  }
...
server.listen(PORT, () => {
  console.log(`AlgoPe Weather API listening on http://localhost:${PORT}`)
  console.log(`  Try: curl "http://localhost:${PORT}/weather?city=London"`)
  console.log('')
  console.log('Next: point `algope init` at this backend URL, then `algope register`.')
})
