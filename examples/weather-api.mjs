 ...
 *   2. Register with AlgoPe: algope register
 *   3. Start x402 proxy: algope start
 *   4. Query via agent: algope-agent run "What's the weather in London?"
 */
...
Next steps:
  1. Register service:  cd ../packages/algope && node dist/cli.js register
  2. Start x402 proxy:  cd ../packages/algope && node dist/cli.js start
  3. Test with agent:   algope-agent run "What's the weather in London?"
`);
});
