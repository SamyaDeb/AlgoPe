/**
 * AlgoPe x402 Proxy — Railway deployment
 *
 ...
 *   SERVICE_NAME      Display name (default: "AlgoPe Service")
...
const SERVICE_NAME  = process.env.SERVICE_NAME ?? 'AlgoPe Service'
const PORT          = parseInt(process.env.PORT ?? '4402', 10)
// "goplausible" settles through the hosted facilitator: gasless for callers
// and the resource becomes visible in the facilitator's Bazaar catalog.
// "self" relays the client-signed group here instead, with no third party.
const FACILITATOR_MODE = (process.env.FACILITATOR_MODE ?? 'goplausible').toLowerCase()
...
const settler = FACILITATOR_MODE === 'self'
  ? new SimpleVerifier()
  : new HTTPFacilitatorClient({ url: FACILITATOR_URL })
const resourceServer = new x402ResourceServer(settler)
...
app.listen(PORT, () => {
  console.log(`AlgoPe proxy started`)
  console.log(`  Service:  ${SERVICE_NAME}`)
  console.log(`  Target:   ${TARGET_URL}`)
  console.log(`  Price:    ${PRICE} ${PAYMENT_TOKEN}`)
  console.log(`  Network:  ${NETWORK}`)
  console.log(`  Settle:   ${FACILITATOR_MODE === 'self' ? 'self-relay (no facilitator)' : FACILITATOR_URL}`)
